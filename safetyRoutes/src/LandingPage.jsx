...
const translations = {
  en: {
    howItWorks: "How it Works", fileReport: "File a Report", aboutUs: "About Us",
    login: "Login", signUp: "Sign Up", title: "SafeYatra",
    mission: "SafeYatra", missionSub: "Real-time safety insights, community reports, and smart routing — all in one place.",
    getDirections: "Explore the Map", footer: "Crafted by team Gradient Gang",
    safetyTip: "Safety Tip", english: "EN", hindi: "हि",
  },
  hi: {
    howItWorks: "यह कैसे काम करता है", fileReport: "रिपोर्ट दर्ज करें", aboutUs: "हमारे बारे में",
    login: "लॉग इन", signUp: "साइन अप", title: "सेफयात्रा",
    mission: "विश्वास के साथ यात्रा करें।", missionSub: "रीयल-टाइम सुरक्षा जानकारी, समुदाय रिपोर्ट और स्मार्ट रूटिंग।",
    getDirections: "मानचित्र देखें", footer: "टीम ग्रेडिएंट गैंग द्वारा निर्मित",
    safetyTip: "सुरक्षा टिप", english: "EN", hindi: "हि",
  }
};
...
const RouteAnimation = ({ light }) => {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-10 overflow-hidden">
      <svg
        style={{ position: 'absolute', left: '28%', top: '6%', width: 'clamp(220px, 52vw, 660px)', height: 'auto' }}
        viewBox="0 0 300 150" preserveAspectRatio="xMidYMid meet"
      >
        <defs>
        ...
        <motion.path
          ...
          transition={{ duration: 2.5, ease, repeat: Infinity, repeatType: 'reverse', repeatDelay: 0.3 }}
        />
        <motion.path
          d="M20,140 L80,100 L140,140 L200,100 L260,140 L280,60"
          fill="none" stroke={stroke} strokeWidth="2" strokeDasharray="8 5"
          strokeLinecap="round" strokeLinejoin="round" filter="url(#glow)"
          initial={{ pathLength: 0, opacity: 0 }} animate={{ pathLength: 1, opacity: 1 }}
          transition={{ duration: 2.5, ease, repeat: Infinity, repeatType: 'reverse', repeatDelay: 0.3, opacity: { duration: 0.4 } }}
        />
        <motion.circle cx="20" cy="140" r="4" fill={stroke}
          initial={{ scale: 0 }} animate={{ scale: [0, 1.4, 1] }}
          transition={{ duration: 0.5, delay: 0.2 }} />
        <motion.circle cx="20" cy="140" r="4" fill="none" stroke={stroke} strokeWidth="1.5"
          animate={{ r: [4, 10], opacity: [0.6, 0] }}
          transition={{ duration: 1.5, repeat: Infinity, repeatDelay: 1 }} />
        <motion.circle cx="280" cy="60" r="4" fill={stroke}
          initial={{ scale: 0 }} animate={{ scale: [0, 1.4, 1] }}
          transition={{ duration: 0.5, delay: 2.3, repeat: Infinity, repeatDelay: 2.8 }} />
        <motion.circle cx="280" cy="60" r="4" fill="none" stroke={stroke} strokeWidth="1.5"
          animate={{ r: [4, 11], opacity: [0.7, 0] }}
          transition={{ duration: 1.6, delay: 2.4, repeat: Infinity, repeatDelay: 1.2 }} />
      </svg>
    </div>
  );
};
...
const ProfileMenu = ({ light, t, navigate }) => {
  ...
  const itemBase = 'w-full text-left px-3 py-2.5 text-sm font-medium transition-all duration-150 flex items-center gap-3 rounded-xl';
  const itemCls = light ? `${itemBase} text-gray-700 hover:bg-gray-50` : `${itemBase} text-gray-300 hover:bg-white/6`;
    ...
    <div ref={ref} className="relative">
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            {/* Account header */}
            <div className={`flex items-center gap-3 px-3 py-2.5 mb-1.5 rounded-xl ${light ? 'bg-gray-50' : 'bg-white/5'}`}>
              <div className="w-9 h-9 rounded-full overflow-hidden ring-2 ring-white/20 flex-shrink-0">
                <img src={profileIcon} alt="profile" className="w-full h-full object-cover" />
              </div>
              <div className="min-w-0">
                <p className={`text-sm font-semibold truncate ${light ? 'text-gray-900' : 'text-white'}`}>My Account</p>
                <p className={`text-xs truncate ${light ? 'text-gray-400' : 'text-gray-500'}`}>View profile</p>
              </div>
            </div>

            <button className={itemCls} onClick={() => { navigate('/accounts'); setOpen(false); }}>
              <span className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${light ? 'bg-gray-100' : 'bg-white/8'}`}>
                <svg width="14" height="14" viewBox="0 0 15 15" fill="none">
                  <circle cx="7.5" cy="5" r="2.5" stroke="currentColor" strokeWidth="1.3" />
                  <path d="M2 13c0-2.8 2.46-5 5.5-5s5.5 2.2 5.5 5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" />
                </svg>
              </span>
              Profile
            </button>

            <button className={itemCls} onClick={() => { navigate('/settings'); setOpen(false); }}>
              <span className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${light ? 'bg-gray-100' : 'bg-white/8'}`}>
                <svg width="14" height="14" viewBox="0 0 15 15" fill="none">
            ...
            >
              <span className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${light ? 'bg-red-50' : 'bg-red-900/20'}`}>
                <svg width="14" height="14" viewBox="0 0 15 15" fill="none">
                  <path d="M5 2H2v11h3M10 4l3.5 3.5L10 11M5.5 7.5h8" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </span>
              Sign out
            </button>
    ...
    <div ref={ref} className="relative md:hidden z-50">
      <button onClick={() => setOpen(!open)} aria-label="Menu"
        ...
        {[0, 1, 2].map(i => (
          <motion.span key={i}
            animate={open
              ? i === 1 ? { opacity: 0, scaleX: 0 } : { rotate: i === 0 ? 45 : -45, y: i === 0 ? 7 : -7 }
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div className="py-2">
              {[{ l: t.howItWorks, p: '/how' }, { l: t.fileReport, p: '/file' }, { l: t.aboutUs, p: '/aboutus' }].map(({ l, p }, i) => (
                <motion.button key={p}
              ...
              {[{ l: t.login, p: '/login' }, { l: t.signUp, p: '/signup' }].map(({ l, p }, i) => (
                <motion.button key={p}
                  initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }}
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <span className={`w-1.5 h-1.5 rounded-full animate-pulse ${light ? 'bg-blue-500' : 'bg-amber-400'}`} />
          <span className={`text-xs font-semibold tracking-widest uppercase ${light ? 'text-blue-600' : 'text-amber-400'}`}>
            {t.safetyTip}
          </span>
        </div>
      ...
      <div className="flex gap-1 mt-2.5">
        {tips.map((_, i) => (
          <motion.div key={i}
            animate={{ width: i === idx ? 14 : 4, opacity: i === idx ? 1 : 0.3 }}
            transition={{ duration: 0.35 }}
...
const LandingPage = () => {
  ...
  return (
    ...
    >
      ...
      {!light && <div className="absolute inset-0 z-0 bg-gradient-to-b from-[#080C14]/70 via-[#080C14]/40 to-[#080C14]/90 pointer-events-none" />}
      {light && <div className="absolute inset-0 z-0 bg-gradient-to-b from-white/30 via-transparent to-white/60 pointer-events-none" />}
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-14 grid grid-cols-3 items-center">
          ...
          <motion.div
            ...
            onClick={() => navigate('/')}
          >
            <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${light ? 'bg-gray-900' : 'bg-white/15 border border-white/20'}`}>
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                <path d="M7 1L2 3v4c0 3 2.5 5 5 6 2.5-1 5-3 5-6V3L7 1z" fill="white" />
              </svg>
            </div>
            <span className={`text-lg font-bold tracking-tight ${logoColor}`}>{t.title}</span>
          </motion.div>

          {/* CENTRE: Nav links — perfectly symmetric */}
          <div className="hidden md:flex items-center justify-center gap-7">
            {[{ l: t.howItWorks, p: '/how' }, { l: t.fileReport, p: '/file' }, { l: t.aboutUs, p: '/aboutus' }].map(({ l, p }) => (
              <motion.button
                key={p}
                whileHover={{ y: -1 }}
                onClick={() => navigate(p)}
          ...
          {/* RIGHT: Auth pill + theme toggle + profile */}
          <div className="flex items-center justify-end gap-2.5">

            {/* Auth pill group — Login ghost | Sign Up solid */}
            <div className={`hidden md:flex items-center rounded-xl p-1 gap-1 ${light ? 'bg-black/5' : 'bg-white/8 border border-white/10'}`}>
              <motion.button
                whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
                onClick={() => navigate('/login')}
                className={`h-7 px-4 rounded-lg text-xs font-semibold transition-all duration-200 ${
                  light ? 'text-gray-600 hover:bg-white hover:text-gray-900 hover:shadow-sm' : 'text-gray-400 hover:bg-white/10 hover:text-white'
                }`}
              >
                {t.login}
              </motion.button>
            ...
            {/* Theme toggle — animated spring thumb */}
            <button
      ...
      <main className="relative z-20 flex-1 flex flex-col items-center justify-center text-center px-4 sm:px-6 min-h-0">
        <motion.div variants={container} initial="hidden" animate={mounted ? "show" : "hidden"} className="w-full max-w-3xl mx-auto">

          <motion.div variants={item} className="flex justify-center mb-4">
            <span className={`inline-flex items-center gap-2 text-xs font-semibold tracking-widest uppercase px-4 py-1.5 rounded-full border ${
              light ? 'bg-white/80 border-gray-200 text-gray-600' : 'bg-white/8 border-white/15 text-gray-300'
            }`}>
              <span className={`w-1.5 h-1.5 rounded-full animate-pulse ${light ? 'bg-emerald-500' : 'bg-emerald-400'}`} />
          ...
          <motion.div variants={item} className="flex flex-col sm:flex-row gap-3 justify-center items-center mb-8">
            <motion.button
              ...
              <svg width="15" height="15" viewBox="0 0 16 16" fill="none">
                <path d="M8 1C5.2 1 3 3.2 3 6c0 4 5 9 5 9s5-5 5-9c0-2.8-2.2-5-5-5zm0 6.5a1.5 1.5 0 110-3 1.5 1.5 0 010 3z" fill="currentColor" />
              </svg>
              {t.getDirections}
            </motion.button>
            <motion.button
          ...
          <motion.div variants={item} className="grid grid-cols-3 gap-3 max-w-sm mx-auto">
            {stats.map(({ value, label }) => (
              <div key={label} className={`rounded-xl p-3 text-center transition-colors duration-300 ${statCard}`}>
                <div className={`text-lg sm:text-xl font-bold tracking-tight ${heroTitle}`}>{value}</div>
                <div className={`text-xs mt-0.5 leading-tight ${heroSub}`}>{label}</div>
              </div>
            ))}
          </motion.div>
        </motion.div>
      ...
      <motion.footer
        ...
        className={`relative z-30 w-full flex-shrink-0 py-2.5 px-4 transition-all duration-300 ${
          light ? 'bg-white/80 backdrop-blur-xl border-t border-black/8' : 'bg-[#080C14]/80 backdrop-blur-xl border-t border-white/8'
        }`}>
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-3 text-xs">

          {/* Left: credit */}
          <p className={light ? 'text-gray-400' : 'text-gray-500'}>{t.footer}</p>

          {/* Centre: language toggle */}
          <div className={`flex items-center gap-0.5 rounded-lg p-1 font-semibold ${light ? 'bg-gray-100' : 'bg-white/8 border border-white/10'}`}>
            {['en', 'hi'].map(l => (
              <motion.button
                key={l}
                whileTap={{ scale: 0.95 }}
                onClick={() => setLang(l)}
                className={`px-2.5 py-1 rounded-md text-xs transition-all duration-200 ${lang === l
                  ? (light ? 'bg-white text-gray-900 shadow-sm' : 'bg-white/20 text-white')
          ...
          <div className="flex items-center gap-4">
            {['Privacy', 'Terms', 'Contact'].map(link => (
              <button key={link} className={`transition-colors duration-200 ${light ? 'text-gray-400 hover:text-gray-700' : 'text-gray-500 hover:text-gray-300'}`}>
                {link}
              </button>
            ))}
          </div>
