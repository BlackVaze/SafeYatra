...
/* ── Exact same canvas 3D background as HowItWorks ── */
function use3DCanvas(canvasRef, dark) {
  useEffect(() => {
    ...
    function drawGrid(t) {
      ...
      const cols = 14, rows = 10, spread = w*1.2;
    ...
    function drawBlobs(t) {
      ...
      const blobs = dark
        ? [{x:.15,y:.15,r:.35,c:'16,185,129',a:.10},{x:.85,y:.28,r:.28,c:'37,99,235',a:.08},{x:.4,y:.78,r:.22,c:'124,58,237',a:.07}]
        : [{x:.15,y:.15,r:.30,c:'16,185,129',a:.07},{x:.88,y:.25,r:.25,c:'37,99,235',a:.05},{x:.45,y:.78,r:.20,c:'124,58,237',a:.04}];
    ...
    function drawCubes() {
      for (const c of cubeStates) {
        ...
        const px=c.cx*canvas.width, py=c.cy*canvas.height;
        ctx.strokeStyle=`rgba(${c.col},${c.a})`; ctx.lineWidth=1.2;
    ...
    const particles=Array.from({length:40},(_,i)=>({
      ...
      col:dark?(i%5===0?'37,99,235':i%7===0?'124,58,237':'16,185,129'):(i%5===0?'29,78,216':i%7===0?'109,40,217':'15,118,110'),
    ...
    function draw() {
      ...
      drawBlobs(t); drawGrid(t); drawCubes();
      for (const p of particles) {
        p.y-=p.speed; if(p.y<-0.02) p.y=1.02;
        ...
        ctx.fillStyle=`rgba(${p.col},${Math.max(0,alpha)})`; ctx.fill();
      }
...
/* ── Crime categories ── */
const CRIMES = [
  { group:'Violent Crimes', options:['Homicide','Crim Sexual Assault','Criminal Sexual Assault','Kidnapping','Human Trafficking'] },
  { group:'Crimes Against Persons', options:['Robbery','Arson','Assault','Battery','Weapons Violation','Intimidation','Stalking'] },
  { group:'Property Crimes', options:['Burglary','Motor Vehicle Theft','Theft','Criminal Damage','Criminal Trespass'] },
  { group:'Other Offenses', options:['Offense Involving Children','Sex Offense','Prostitution','Narcotics','Liquor Law Violation','Gambling','Public Peace Violation','Obscenity','Public Indecency','Concealed Carry License Violation','Deceptive Practice','Interference With Public Officer','Other Offense','Other Narcotic Violation','Non-Criminal','Ritualism'] },
];

/* ── Spinner icon ── */
const Spinner = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"
    style={{ animation:'spin .7s linear infinite', display:'inline-block', verticalAlign:'middle', marginRight:8 }}>
...
const FileReport = () => {
  ...
  const labelStyle = {
    ...
    color: t2, marginBottom:8,
    transition:'color .25s',
  };

  /* Char bar helpers */
  const charPct = (description.length / 1000) * 100;
  const charBarClass = description.length > 900 ? 'char-bar-fill danger'
                     : description.length > 700 ? 'char-bar-fill warn'
                     : 'char-bar-fill';
  const charCountColor = description.length > 900 ? '#ef4444'
                       : description.length > 700 ? '#f59e0b'
                       : t3;
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const response = await API.post('/api/reports/submit', { location, time, crime, description });
      Toast.success(response.data.message);
      setSubmitted(true);
      setTimeout(() => setSubmitted(false), 2800);
      setLocation(''); setTime(''); setCrime(''); setDesc('');
    } catch {
    ...
    <>
      ...
      <div className="sy" style={{ background:bg, minHeight:'100vh', display:'flex', flexDirection:'column', color:t1, transition:'background .3s,color .3s', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'relative', zIndex:1, display:'flex', flexDirection:'column', minHeight:'100vh' }}>
          ...
          {/* ── MAIN ── */}
          <main style={{ flex:1, maxWidth:560, margin:'0 auto', width:'100%', padding:'8px 20px 64px' }}>

            {/* Hero label */}
            <header style={{ textAlign:'center', padding:'24px 0 36px', animation:'fadeUp .5s cubic-bezier(.22,1,.36,1) both' }}>
              
              <h1 style={{ fontSize:'clamp(1.5rem,4vw,2.1rem)', fontWeight:800, lineHeight:1.2, margin:'0 0 10px', letterSpacing:'-.03em' }}>
                File a <span style={{ color:G }}>Report</span>
              </h1>
              <p style={{ fontSize:14, color:t2, lineHeight:1.7 }}>
                Help keep your community safe by reporting incidents. All reports are confidential.
              </p>
            </header>

            {/* ── FORM CARD ── */}
            <div className="form-card fu1" style={{ borderRadius:20, padding:'36px 32px', ...glass, boxShadow: dark ? '0 24px 64px rgba(0,0,0,.5), 0 0 0 1px rgba(16,185,129,.06)' : '0 8px 32px rgba(0,0,0,.08)' }}>

              {/* Success flash */}
              {submitted && (
                <div style={{ display:'flex', alignItems:'center', gap:10, background:'rgba(16,185,129,.12)', border:'1px solid rgba(16,185,129,.3)', borderRadius:12, padding:'12px 16px', marginBottom:24, animation:'scaleIn .3s cubic-bezier(.22,1,.36,1) both' }}>
                  <svg className="success-check" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={G} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                    <polyline points="20 6 9 17 4 12"/>
                  </svg>
                  <span style={{ fontSize:13, color:G, fontWeight:600 }}>Report submitted successfully!</span>
                </div>
              )}
              ...
              <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:22 }}>
                ...
                <div className="fu2 field-wrap">
                  <label className="field-label" style={labelStyle}>
                    Location<span className="req-dot" title="required" />
                  </label>
                  <div style={{ position:'relative' }}>
                    ...
                    />
                    <span className="field-icon" style={{ position:'absolute', right:13, top:'50%', transform:'translateY(-50%)', fontSize:16, lineHeight:1 }}>📍</span>
                  </div>
                </div>
                ...
                <div className="fu3 field-wrap">
                  <label className="field-label" style={labelStyle}>
                    Time of Incident<span className="req-dot" title="required" />
                  </label>
                  <div style={{ position:'relative' }}>
                    <input
                      type="text"
                      ...
                      style={{ ...inputStyle, paddingRight:42 }}
                    />
                    <svg className="field-icon" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={t3} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
                ...
                <div className="fu4 field-wrap">
                  <label className="field-label" style={labelStyle}>
                    Crime Type<span className="req-dot" title="required" />
                  </label>
                  <select
                    value={crime}
                ...
                <div className="fu5 field-wrap">
                  <label className="field-label" style={{ ...labelStyle, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    <span>Description<span className="req-dot" title="required" /></span>
                    <span style={{ fontSize:11, color: charCountColor, fontWeight:500, textTransform:'none', letterSpacing:0, transition:'color .25s' }}>
                  ...
                  <textarea
                    ...
                    required
                    className="sy-input"
                    style={{ ...inputStyle, resize:'vertical', minHeight:120, lineHeight:1.65 }}
                  />
                  {/* Progress bar */}
                  <div className="char-bar-track">
                    <div className={charBarClass} style={{ width:`${charPct}%` }} />
                  </div>
                </div>

                {/* Divider */}
                <div style={{ height:1, background:border, borderRadius:99 }} />
                ...
                >
                  {submitting ? <><Spinner />Submitting…</> : submitted ? '✓ Submitted!' : 'Submit Report →'}
                </button>
            ...
            {/* Bottom note */}
            <p className="fu6" style={{ textAlign:'center', fontSize:12, color:t3, marginTop:20, lineHeight:1.6 }}>
              Your report is encrypted and confidential. SafeYatra does not share your information without consent.
            </p>
