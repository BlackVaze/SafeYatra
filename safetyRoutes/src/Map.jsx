...
export default function Map() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Resolve start — use GPS if empty, otherwise geocode
    let startCoords;
    if (!startLocation.trim()) {
      try {
        startCoords = await new Promise((res, rej) => {
          ...
          navigator.geolocation.getCurrentPosition(
            (p) => res([p.coords.latitude, p.coords.longitude]),
            () => rej(new Error("Could not get your location. Please enter a start address.")),
            { enableHighAccuracy: true, timeout: 8000 },
          );
        });
      } catch (err) {
    ...
    // Resolve end — geocode the destination address
    if (!endLocation.trim()) {
    ...
    let ep;
    try {
      ep = await geocodeAddress(endLocation);
    } catch (err) {
      showToast(err.message);
      return;
    }
